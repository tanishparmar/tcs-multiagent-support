...
def _query(sql: str, params: dict) -> str | None:
    """Execute a parameterized query and return formatted rows."""
    ...
    return "\n".join(str(dict(zip(keys, row))) for row in rows) if keys else "\n".join(str(r) for r in rows)
...
def search_customers(query: str) -> str:
    ...
    result = _query(
        """SELECT id, name, email, phone, account_type, location, join_date, loyalty_points
           FROM customers
           WHERE name LIKE :p OR email LIKE :p OR account_type LIKE :p OR location LIKE :p
           LIMIT 10""",
        {"p": pattern},
    )
    return result or f"No customers found matching '{query}'."
...
def get_customer_tickets(customer_name: str) -> str:
    ...
    return result or f"No tickets found for customer '{customer_name}'."
...
def get_customer_orders(customer_name: str) -> str:
    ...
    return result or f"No orders found for customer '{customer_name}'."
...
def get_customer_full_profile(customer_name: str) -> str:
    ...
    pattern = {"p": f"%{customer_name}%"}

    profile = _query(
        """SELECT id, name, email, phone, account_type, location, join_date, loyalty_points
           FROM customers WHERE name LIKE :p LIMIT 1""",
        pattern,
    )
    if not profile:
...
def get_ticket_statistics() -> str:
    ...
    return result or "No ticket data available."
